...
class SCMessGUI(QMainWindow):
   ...
   # ========================== ТЕМЫ ИНТЕРФЕЙСА ==========================
    ...
    def apply_theme(self, theme):
        ...
        if theme == "dark":
            self.set_fusion_dark_palette(app) # Исправленная строка 1
            return                            # Исправленная строка 2
    ...
    def set_fusion_dark_palette(self, app):
        ...
        dark_palette.setColor(QPalette.HighlightedText, QColor(0, 0, 0)) # Белый текст на фокусе/выделении

        app.setPalette(dark_palette)

    def set_fusion_light_palette(self, app): # <-- Исправлено: добавлено двоеточие, убран параметр по умолчанию
        # Яркие, чистые цвета для светлой темы
        light_palette = QPalette() # <-- Создаем объект палитры внутри функции
        ...
        disabled_text_color = QColor(128, 128, 128) # Серый отключенный текст, исправлена ошибка в предыдущем значении

        light_palette.setColor(QPalette.Window, window_color)
        light_palette.setColor(QPalette.WindowText, window_text_color)
        light_palette.setColor(QPalette.Base, base_color)
        light_palette.setColor(QPalette.AlternateBase, QColor(247, 247, 247)) # Slightly off-white for alternate rows
        ...
        app.setPalette(light_palette) # <-- Применяем палитру к переданному объекту приложения
